...
public class Mexc extends Exchange {
    ...
    private final String exchange = "mexc";
    ...
    public Mexc(String apiKey, String secretKey, String passphrase) {
        super("mexc", "https", "api.mexc.com", -1);
        this.apiKey = apiKey;
    ...
    public HttpRequest request(URI uri, String signature, String timestamp, String method) {
        ...
        builder.header("X-MEXC-APIKEY", apiKey);
        if (method == "GET") {
            builder.GET();
        } else if (method == "POST") {
            builder.POST(null);
    ...
    public String requestGET(String endPoint, String query) {
        ...
        if (query != "") {
            query += query + "&recvWindow=" + recvWindow + "&timestamp=" + timestamp;
        } else {
    ...
    private String sign(String message) {
        out.printf("mac sign: %s", mac.doFinal(message.getBytes()).length);
